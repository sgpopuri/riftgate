...
**Status: `v0.2` shipped the systems showpiece, `v0.3` shipped programmability, `v0.4` shipped gateway-internal observability (eBPF runtime wiring, token-level metrics, GPU pressure correlation), and `v0.5` shipped the MCP capability-broker milestone, and **`v1.0` shipped the production-ready milestone: Kubernetes operator with CRDs, `TenantResolver` + `ApiKeyTenantResolver`, property-based parser tests, WAL fuzz target, and CRD-driven API key registry. Distribution remains build-from-source; crates.io publish is deferred.** The `v0.0` design surface — vision, requirements, four-plane architecture (data, control, extension, observability), low-level design notes, Options docs, and foundational ADRs — landed first. `v0.1` shipped the walking-skeleton crates (`riftgate-core`, `riftgate-io-epoll`, `riftgate-parser`, `riftgate-config`, `riftgate-router`, `riftgate-obs`, and the `riftgate` binary) and a self-contained [`examples/01-basic-openai-proxy`](examples/01-basic-openai-proxy/) dev loop. `v0.2` then added the `PerCoreScheduler`, `TokenBucketLimiter`, `WeightedRandomRouter`, `CircuitBreakerArbiter`, `HighWaterPolicy`, the new [`crates/riftgate-replay`](crates/riftgate-replay/) `FileWal`, and the [`crates/riftgate-io-uring`](crates/riftgate-io-uring/) scaffold. `v0.3` closed the programmability milestone with the native `FilterChain` executor, production `WasmFilter` runtime, `KvAwareRouter`, `HedgedRouter`, stream cancellation primitives, and the `riftgate-replay` CLI. `v0.4` closed the observability milestone with `TokenLevelAggregator`, `DcgmScrapeSource`, feature-gated `NvmlSource`, `BpfSink` runtime wiring, staged Aya object workflows, and GPU-aware routing signal integration. `v0.5` closed the agentic capability-plane milestone with `crates/riftgate-mcp` (`AllowlistBroker`, `DryRunBroker`, MCP parser, HMAC attestation, WAL audit), POST `/mcp` authorization path in the binary, and the Lima VM dev environment. Retrospectives: [`v0.0`](docs/02a-v0.0-retrospective.md), [`v0.1`](docs/02b-v0.1-retrospective.md), [`v0.2`](docs/02c-v0.2-retrospective.md), [`v0.3`](docs/02d-v0.3-retrospective.md), [`v0.4`](docs/02e-v0.4-retrospective.md), [`v0.5`](docs/02f-v0.5-retrospective.md).
