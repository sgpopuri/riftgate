...
>
> - **Active milestone:** `v0.5` planning and sequencing (queued). `v0.3` Programmability and `v0.4` eBPF and the depths are **landed and closed-out** (see [`v0.3` retrospective](02d-v0.3-retrospective.md) and [`v0.4` retrospective](02e-v0.4-retrospective.md)).
...
> - **Environment baseline (implementation execution context):** development currently targets a local Ubuntu VM profile recorded only in ignored local config. That environment remains the single default machine class for implementation, build, and test loops. No outbound web is assumed; system dependencies come from `apt`, cargo registry overrides are rendered into ignored local config files, Docker Engine + Compose are available for local example and smoke-loop validation when images are already present locally, and Git-only sources must be staged into the environment by copy when needed.
