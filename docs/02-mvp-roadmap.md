...
>
> - **Active milestone:** `v0.5` — MCP capability broker — **landed and closed-out** (see [`v0.5` retrospective](02f-v0.5-retrospective.md)). ADR 0015 accepted 2026-07-04. Full v0.5+carry-forward surface landed and verified. **v0.4 eBPF resolved (2026-07-04):** Aya BPF programs compiled on Lima VM; all 4 verifier harness tests green. Script fixes: nightly detection via `rustup toolchain list`, `.bpf.o` rename, EM_BPF `Linux BPF` string. **Options 017 + ADR 0029 accepted (2026-07-04):** `TenantResolver` trait in `riftgate-core`, `HeaderTenantResolver`, `ApiKeyTenantResolver` (SHA-256 key hashing, `[multitenancy.api_keys]` config), `MultitenancyConfig` in `riftgate-config`; per-request tenant identity now routes through the resolver in `proxy.rs`. **Active focus: `v1.0` — K8s operator (Options 018 next).**
