...
>
> - **Active milestone:** `v0.5` — MCP capability broker — **landed and closed-out** (see [`v0.5` retrospective](02f-v0.5-retrospective.md)). ADR 0015 accepted 2026-07-04. Full v0.5+carry-forward surface landed and verified. **v0.4 eBPF resolved (2026-07-04):** Aya BPF programs compiled on Lima VM; all 4 verifier harness tests green. **Options 017 + ADR 0029 accepted (2026-07-04):** `TenantResolver` trait, `HeaderTenantResolver`, `ApiKeyTenantResolver` (SHA-256 key hashing). **Options 018 + ADR 0030 accepted (2026-07-04):** K8s operator with CRDs (`Riftgate`, `RiftgateBackend`, `RiftgateRoute`) using `kube-rs`; Helm chart as distribution layer. **Active focus: `v1.0` — implement `crates/riftgate-operator` (ADR 0030).**
