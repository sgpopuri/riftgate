...
>
> - **Active milestone:** `v0.5` — MCP capability broker — **landed and closed-out** (see [`v0.5` retrospective](02f-v0.5-retrospective.md)). ADR 0015 accepted 2026-07-04. `crates/riftgate-mcp` shipped with full parser, `AllowlistBroker` (bit-set + prefix-set + time-bounded grants), `DryRunBroker`, HMAC attestation, WAL audit (FileWal + NoopWal fallback), tenant name hashing (FNV-1a). Config schema, binary wiring (POST `/mcp` authorization path, attestation header injection), Lima VM dev environment, and full workspace clippy+test verification all landed in the same session. **Active focus: `v1.0` planning.**
