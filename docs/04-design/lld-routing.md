...
| `WeightedRandomRouter` | **v0.2** (+ v0.4 signal fold-in) | `riftgate-router` | Walker alias method (Vose 1991); O(1) sampling regardless of weight distribution; alias table rebuilt at config-load and config-reload. Capped at N = 32 eligible backends in v0.2. In v0.4, route-time candidate selection now folds `BackendSignal.gpu_pressure`: prefers closed + cooler backends when available, with safe fallback to any closed backend if all are hot. Per [ADR `0014`](../06-adrs/0014-weighted-random-router.md). |
...
- **GPU-pressure signal is advisory, not absolute.** In v0.4, weighted routing deprioritizes backends at high `gpu_pressure` when cooler closed backends exist; it does not hard-reject hot backends. This prevents pathological reject behavior when every closed backend is hot and preserves liveness under cluster-wide pressure.
